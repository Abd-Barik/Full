...
async function loadItemData(itemRno) {
    ...
    try {
        ...
        const response = await fetch(`/api/itemmaster/service_item_master_view/${itemRno}/`, {
            method: 'GET',
            headers: {
...
function populateInput(input, value, type) {
    ...
    if (input.hasAttribute('data-provider') && input.getAttribute('data-provider') === 'flatpickr') {
        if (value) {
            const formattedDate = formatDateForFlatpickr(value);
            input.value = formattedDate;
            if (input._flatpickr) {
                input._flatpickr.setDate(formattedDate, false);
            }
        }
    }
}
    ...
    if (mode === 'edit') {
        ...
        if (saveButton) {
            saveButton.addEventListener('click', function(e) {
                ...
                fetch(`/api/itemmaster/service_item_master_update/${itemRno}/`, {
                    method: 'POST',
                ...
                .finally(() => {
                    ...
                    this.disabled = false;
                    window.location.href = "/itemmaster/service_item_master/";
                });
            });
        }
    }else {
        ...
        if (saveButton) {
            saveButton.addEventListener('click', function(e) {
                ...
                fetch('/api/itemmaster/service_item_master_form_submit/', {
                    method: 'POST',
                    body: formData,
                    headers: {
                        'X-CSRFToken': document.querySelector('[name=csrfmiddlewaretoken]').value
